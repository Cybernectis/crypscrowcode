...
class UserRepository extends BaseRepository
{
     ...
     * @throws GeneralException
     */
    public function findByPhoneOtp($otp)
    {
        $user = $this->model
            ->where('phone_otp', $otp)
    ...
    public function create(array $data)
    {
        return DB::transaction(function () use ($data) {
            ...
            $user = parent::create([
                ...
                'gender'            => $data['gender'],
                'refer_by_user_id'  => $data['refer_by'],
                'email'             => $data['email'],
                "google2fa_secret"  =>  '',
            ...
            if ($user) {
                ...
                /*
                *Generate and add user coins wallet adress 
                */
                $allLocalCoins = LocalCoin::all();
                foreach ($allLocalCoins as $key => $value) 
                {
                    $coinName = strtolower($value->short_name);
                    $apiContext = Helper::blockCypherConfig($coinName);
                    ...
                    try {
                        ...
                        if (!empty($addressKeyChain))
                        {
                            ...
                            $wallet->local_coins_id             =   $value->id;
                            $wallet->private_key                =   $response->private;
                            $wallet->public_key                 =   $response->public;
                            $wallet->wallet_address             =   $response->address;
                            $wallet->save();
                            // echo $wallet->address;
                        }
                    } catch (Exception $ex) {
            ...
            if (config('access.users.confirm_email')) {
                ...
                $user->notify(new UserNeedsConfirmation($user->confirmation_code,$user->username));
            }
    ...
    public function update($id, array $input, $image = false)
    {
        ...
        $user->authentication_type = $input['authentication_type'];
        
        $user->blurb = $input['blurb'];
        $user->avatar_type = $input['avatar_type'];
        $user->google2fa_secret = !empty($input['google2fa_secret']) ? $input['google2fa_secret'] : '';
     ...
     * @throws GeneralException
     */
    public function confirmPhone($otp)
    {
        $user = $this->findByPhoneOtp($otp);
	 ...
	 public function confirmPhoneProfile($otp,$phone,$country_code)
    {
        $user = $this->findByPhoneOtp($otp);
